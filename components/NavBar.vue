...
<template>
	<nav class="z-30 navPadding flex items-center justify-between sticky top-0 w-full shadow" :class="navBgTextColor" id="navBar">
		<div>
			<button type="button" class="flex items-center text-2xl font-bold cursor-pointer" @click="goToHome">
		...
		<div v-if="!loading" class="hidden lg:flex font-medium">
			<transition-group tag="span" appear @before-enter="before" @enter="entering" class="space-x-5 lg:space-x-8 xl:space-x-10">
				<a v-for="item in navigation" :key="item.id" :href="item.href" class="text-lg lg:text-lg xl:text-xl headerFont underAnimation">{{
					item.name
				}}</a>
			</transition-group>
...
<script setup>
	...
	watchEffect(() => {
		if (currentOffsetHeight.value > screenHeight.value - heightOfNav.value * 2.5 && route.name === "index") {
